...
- `CaseStructuringAssembler → CaseStructuringResult`：最终输出由代码创建，而不是由 LLM 直接生成。
...
这个 agent 不持久化状态。`CaseStructurerAgent.run()` 返回的是一个 `CaseStructuringResult` 对象，调用方或未来状态写入模块需要自己保存它。
...
`SourceSpanResolver` 会在完整 `raw_text` 中查找这段文本。如果找到了，就填充字符位置：
...
当前解析方式使用 `raw_text.find(quoted_text)`，也就是寻找第一个完全匹配的位置。遇到重复短语时，它不会做复杂 disambiguation。
...
flowchart TD
    ...
    L --> M[CaseStructuringResult]
```

这张图强调两点：

- LLM 负责从文本中抽取语义候选对象。
- 最终 `CaseStructuringResult` 由代码组装和校验。
