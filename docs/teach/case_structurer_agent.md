...
这些属于后续 Evidence Atomizer、Hypothesis、Conflict、Action 或 Arbitration 相关模块的职责。
...
raw_text
  ...
  -> deterministic source-span correction
  -> final source-span validation
...
初始结构化结果会进入 `validate_and_correct_source_spans()`。第一步是验证：
...
校正过程会形成 `correction_report`。

### 5.5 最终验证与返回

校正后的结果会再次验证，形成 `final_validation_report`。
...
- 追加 source-span validation/correction bundle 到 `state.source_span_validation_correction_results`。
...
`ClinicalSection` 是粗粒度临床段落，例如现病史、影像、实验室检查、治疗史。它只负责分段和段落分类，不表示 evidence atom。
...
- `SourceSpanResolver` 会根据原文做确定性解析。
- `validate_and_correct_source_spans()` 会验证 span 是否有效。
