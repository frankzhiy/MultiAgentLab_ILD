"""Deferred structured item schema for the Evidence Atomizer.

DeferredStructuredItem represents a StructuredClinicalItem that cannot safely
become an EvidenceAtom. Deferring an item is safer than forcing ambiguous or
...
class DeferredStructuredItem(BaseModel):
    """Structured item that the Evidence Atomizer intentionally deferred.

    This object records atomization safety and provenance only. It must not
    ...
    item_id: ItemID = Field(
        ...,
        description="StructuredClinicalItem id that could not be atomized safely.",
    )
    ...
    explanation: str = Field(
        ...,
        min_length=1,
        description="Atomization-scope explanation for deferring this item.",
    )
