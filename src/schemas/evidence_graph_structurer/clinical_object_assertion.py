"""ClinicalObjectAssertion schema for the Evidence Graph Structurer.

This is the typed payload produced by the Clinical Assertion Resolver. Each
assertion describes one source-grounded clinical object plus its own
intrinsic context (assertion status, temporal anchor, trigger, modifiers).

Downstream structure — frames, evidence nodes, and typed relations between
them — is built by EvidenceFrameAssembler, EvidenceRelationExtractor, and
EvidenceGraphComposer, NOT by adding parent/child fields to the assertion.
"""
